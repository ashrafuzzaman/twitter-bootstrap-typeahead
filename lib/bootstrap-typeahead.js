...
//  https://github.com/ashrafuzzaman/twitter-bootstrap-typeahead
//
    ...
    Typeahead.prototype = {
        ...
        render: function (items) {
            ...
            items = $(items).map(function (i, item) {
            		var keys = that.options.item.match(/#\{\w*\}/gi);
            		var _item = that.options.item;
            		if(keys){
            			for (var i = keys.length - 1; i >= 0; i--) {
	            			var key = keys[i].substring(2,keys[i].length-1);
	            			var val = item[key];
	            			if(key == that.options.display)
	            				val = that.highlighter(item[that.options.display]);
	            			_item = _item.replace("#{" + key + "}", val);
	            		};
            			_item = $(_item).attr('data-value', item[that.options.val]);
            		} else {
            			_item = $(that.options.item).attr('data-value', item[that.options.val]);
                	_item.find('a').html(that.highlighter(item[that.options.display]));
            		}
